...
- **b.cls** · `note = 'if eval fails, try open'` (string): if + try count branch +2 (apex branch includes try) — post-#2545 these two phantom string hits are ALL of b's branch (2); insert/update 2 raw state tripled to 6 by the flux context they create. Ledger: string-literal-selective-shielding, state-flux-branch-weighting
...
- 🔴 `io`: 5 vs median 3 (+67%)
- 🔴 `risk_api_exposure`: 11.5689 vs median 5.29997 (+118%)
...
- 🔴 `risk_api_exposure`: 1.0506 vs median 5.29997 (-80%)
- 🔴 `risk_cognitive_load`: 4 vs median 15.1444 (-74%)
...
- 🔴 `doc`: 2 vs median 1 (+100%)
- 🔴 `risk_api_exposure`: 9.73064 vs median 5.29997 (+84%)
...
- 🟡 `high_risk_execution`: 3 vs median 2 (+50%)
- 🟡 `io`: 4 vs median 3 (+33%)
- 🟡 `risk_documentation`: 75.9927 vs median 54.777 (+39%)
- 🟡 `safety_bypasses`: 3 vs median 2 (+50%)
...
- 🔴 `risk_api_exposure`: 1.89168 vs median 5.29997 (-64%)
...
- **b.kt** · `val note = "if eval fails, try open"` (string): if + try count branch +2 (kotlin branch includes try) — post-#2545 these two phantom string hits are all of b's branch (2); val is not a kotlin state keyword so the decoy line adds no state — var x2 raw tripled to 6. Ledger: string-literal-selective-shielding, state-flux-branch-weighting
...
- 🟡 `risk_api_exposure`: 7.4132 vs median 5.29997 (+40%)
...
- 🔴 `risk_api_exposure`: 11.106 vs median 5.29997 (+110%)
...
- **b.mm** · `NSString *note = @"if eval fails, try open"` (string): if counts branch +1 — post-#2545 that phantom string hit is all of b's branch (1; was 4 = 3 returns + if); the *note = assignment is a 3rd raw state hit, tripled to 9 by flux. Ledger: string-literal-selective-shielding, state-flux-branch-weighting
...
- 🔴 `risk_api_exposure`: 2.4136 vs median 5.29997 (-54%)
...
- 🔴 `doc`: 2 vs median 1 (+100%)
- 🔴 `risk_api_exposure`: 9.28468 vs median 5.29997 (+75%)
...
- 🔴 `risk_cognitive_load`: 28.0116 vs median 15.1444 (+85%)
- 🟡 `globals`: 3 vs median 2 (+50%)
- 🟡 `io`: 4 vs median 3 (+33%)
...
- **b.sol** · `string memory note = "if eval fails, try open"` (string): if + try count branch +2 (solidity includes try) — post-#2545 these two phantom string hits are all of b's branch (2); the note line is a 3rd raw state hit, tripled to 9 by flux. eval inert. Ledger: string-literal-selective-shielding, state-flux-branch-weighting, kotlin-return-in-branch
...
- 🔴 `dependency_links`: 0 vs median 3 (-100%)
- 🔴 `risk_api_exposure`: 0.89698 vs median 5.29997 (-83%)
- 🔴 `risk_documentation`: 9.74282 vs median 54.777 (-82%)
- 🔴 `risk_tech_debt`: 75.87 vs median 39.9315 (+90%)
- 🔴 `safety_bypasses`: 0 vs median 2 (-100%)
...
- 🟡 `keyword_hits`: 308 vs median 242.5 (+27%)
...
- **b.zig** · `var note = "if eval fails, try open"` (string): if + try count branch +2 (zig branch includes try) — post-#2545 these two phantom string hits are all of b's branch (2); the var itself is a state hit and a globals hit (zig globals matches any indented var/const-with-=). var x2 raw state tripled to 6. Ledger: string-literal-selective-shielding, state-flux-branch-weighting
